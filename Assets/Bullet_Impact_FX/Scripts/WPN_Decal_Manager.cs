...
public class WPN_Decal_Manager : Singleton<WPN_Decal_Manager>
{
    ...
    public void SpawnBulletHitEffects(Vector3 position, Vector3 normal, PhysicsMaterial pMat, GameObject hGo)
    {        
        int decalIndex = 0;
        int cIndex = 0;
        bool bFound = false;
        //Debug.Log(hGo.transform.name.ToString());
        Debug.Log(hGo.name.ToString());
        //Debug.Log(hGo.ToString());

        foreach (DictionaryEntry pEntry in physMaterials)
        {
            if (pMat.name.ToString().Contains(pEntry.Value.ToString()))
            {
                decalIndex = int.Parse(pEntry.Key.ToString());//cIndex;
                bFound = true;
                break;
            }
            cIndex++;
        ...
        if ( bFound && m_DecalInfo[decalIndex].m_PFDecals.Length > 0 )
        {
            ...
            if (SpawnAmmo != null)
            {
                ...
                SpawnAmmo.transform.localScale = new Vector3(scaleFact / hGo.transform.localScale.x, scaleFact / hGo.transform.localScale.y, 2f) / 10f;
                //SpawnAmmo.transform.localScale = new Vector3(scaleFact, scaleFact, 2f)/5f;
            }
        }
